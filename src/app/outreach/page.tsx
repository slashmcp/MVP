...
export function OutreachPageContent() {
  ...
  const allRecipients = [
    ...cands.map(c => ({ id: c.id, name: c.name, email: c.email, type: 'Candidate', notes: c.notes })),
    ...clients.map(c => ({ id: c.id, name: c.companyName, email: c.email, type: 'Client', contactPerson: c.contactPerson, notes: c.notes }))
  ];
  ...
  const handleGenerate = async () => {
    if (!recipient) {
      addToast({ type: 'error', message: 'Please select a recipient first' });
      return;
    }
    ...
    try {
      ...
      addToast({ type: 'success', message: 'Email drafted by AI' });
    } catch (e: any) {
      addToast({ type: 'error', message: e.message || 'An error occurred during generation' });
    } finally {
